...
<div class="invoices form large-9 medium-8 columns content" ng-app="invoice_app" ng-controller="invoice_controller">
    ...
    <fieldset>
        ...
        <div class="input date required customInputArea">
            ...
            <input type="date" name="invoice_date" required="required">
        </div>
    ...
    <table style="width: 100%" id="invTbl">
        ...
        <tr ng-repeat="InvoicedItem in InvoicedItems">
            <td> {{$index + 1}} </td>
            <td> <select ng-model="InvoicedItem.item_details" ng-options="x.item_code for x in Items" name="{{'Item_code_' + ($index+1)}}" id="{{'item_select'+($index+1)}}" ng-click="add_dynamic_listener($index+1)" > </select></td>
            <td> {{InvoicedItem.item_details.item_name}} </td>
            <td> <input type="number"  step="any" name="quantity" step="0.5" required="required" ng-model="InvoicedItem.quantity"> </td>
            <td> {{InvoicedItem.item_details.unit}} </td>
            <td>
            	Rp <input id="{{'uang'+($index+1)}}" type="text" ng-keyup="update_price(InvoicedItem, $index+1)">
...
<script>
    ...
    app.controller('invoice_controller', function($scope, $http) {
        ...
        // prepare vars to store items to be invoiced
        $scope.InvoicedItems = [{}];
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        // Format mata uang.
        $('#uang1').mask('000.000.000', {reverse: true});

        $('#item_select1').on('change', function(){
            $('#uang1').val($('#uang1').masked($('#hidden_val1').val()));
        });
